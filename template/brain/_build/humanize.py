...
TWIN COPY: the canonical source of truth is
`$CORE_INSTANCE/scheduling/graphify-brain/humanize.py` (part of the shared
Core-instance baseline, syncs via bin/sync-to-baseline.sh). This file is a
byte-identical twin living in the brain vault repo instead, because
consolidate.py runs out of the vault (a separate git repo per Core / per
fork, not covered by the Core-instance sync manifest) and can't import
across repos without assuming a sibling directory layout that isn't
guaranteed to hold for every fork. If you edit the logic here, port the same
edit to the canonical copy by hand (and vice versa).
