#!/usr/bin/env python3
"""
make-3d.py — emit graph-3d.html from graphify-out/graph.json.

Companion to merge.py's 2D SVG viz. Standalone 3D WebGL renderer using
3d-force-graph (Vasturiano, Three.js). Inlines the graph data so the
file works file:// like graph.html does.

Run after each merge.py:
  python3 scheduling/graphify-brain/make-3d.py

Decision: 2026-05-06 plane session — 3D for navigation/aesthetic; 2D
Canvas (later) for read/traverse. Toggle button in both directions.
"""
import json
import os
import sys
...
def main():
    ...
    nodes = data["nodes"]
    edges = data["edges"]
    lib_path = OUT / "vendor" / "3d-force-graph.min.js"
    ...
    # Compute degree per node for size scaling.
    deg = {}
    for e in edges:
    ...
    # Slim payload — drop heavy properties, keep only what the viewer renders.
    slim_nodes = []
    for n in nodes:
    ...
    # Auto-derive community names: pick the highest-degree node's label per
    # community as a representative hub name. Cluster IDs shift each Louvain
    # run, so curated `community-names.json` (keyed by stale cluster IDs) is
    # not reliable — derive fresh.
    ...
    # Pulled from identity.json (community_naming) so project-specific buckets
    # don't leak into the engine template. See _load_community_buckets_json().
...
def _load_community_buckets_json() -> str:
    ...
    emit a JS-compatible JSON array of {bucket, regex} pairs. Returns "[]"
    if identity.json is missing/malformed — viz falls back to type-based
    bucket assignment (engine-template-friendly).
    """
...
body::before {
  /* subtle radial vignette so the cluster center reads as the focal point */
...
.wordmark .dot {
  ...
  background: radial-gradient(circle at 30% 30%, #a3c2ff, #4a7ed1 60%, #2a4a8a);
  box-shadow: 0 0 12px rgba(122, 166, 255, 0.5);
}
...
#search-box {
  ...
  border: 1px solid var(--border);
  color: var(--fg); padding: 6px 12px; border-radius: 7px;
  width: 280px; outline: none; font-size: 12px;
...
#mode-toggle a {
  padding: 5px 14px; border-radius: 5px; font-size: 11.5px; font-weight: 500;
  color: var(--muted-strong); text-decoration: none; cursor: pointer;
...
/* ── Info panel (right) ────────────────────────────────── */
#info {
  top: 64px; right: 14px; width: 340px; max-height: calc(100vh - 130px);
  overflow-y: auto; padding: 14px 16px; font-size: 12px; display: none;
}
...
/* ── Legend (left) ─────────────────────────────────────── */
#legend {
  top: 64px; left: 14px; width: 230px;
  padding: 12px 14px; font-size: 11.5px; max-height: 44vh; overflow-y: auto;
}
#legend .row {
  display: flex; align-items: center; gap: 9px; padding: 4px 4px;
...
#bottombar {
  ...
  background: linear-gradient(to top, rgba(11,13,18,0.92), rgba(11,13,18,0.55));
  box-shadow: none;
  backdrop-filter: blur(20px);
}
...
/* ── Settings panel ────────────────────────────────────── */
#settings {
  bottom: 56px; right: 14px; width: 280px;
...
#loading, #errbox {
  ...
  padding: 12px 18px; border-radius: 8px; z-index: 99;
}
...
<footer id="bottombar" class="panel">
  <div id="stats"></div>
  <div class="spacer"></div>
  <div id="help-text"><kbd>scroll</kbd> zoom · <kbd>drag</kbd> rotate · <kbd>right-drag</kbd> pan · <kbd>click</kbd> pin node</div>
  <div class="spacer"></div>
...
<div id="settings" class="panel">
  ...
  <div class="row"><label>opacity</label><input type="range" id="s-link-opacity" min="0" max="1" step="0.02" value="0.18"><span class="val" id="s-link-opacity-v">0.18</span></div>
  <div class="row"><label>width</label><input type="range" id="s-link-width" min="0.1" max="3" step="0.05" value="0.5"><span class="val" id="s-link-width-v">0.5</span></div>
  ...
  <div class="row"><label>min size</label><input type="range" id="s-node-min" min="0.2" max="4" step="0.05" value="0.5"><span class="val" id="s-node-min-v">0.50</span></div>
  <div class="row"><label>max size</label><input type="range" id="s-node-max" min="2" max="60" step="0.5" value="40"><span class="val" id="s-node-max-v">40</span></div>
  ...
  <div class="row"><label>resolution</label><input type="range" id="s-node-res" min="4" max="20" step="1" value="8"><span class="val" id="s-node-res-v">8</span></div>
  <div class="row"><label>opacity</label><input type="range" id="s-node-opacity" min="0.3" max="1" step="0.02" value="0.92"><span class="val" id="s-node-opacity-v">0.92</span></div>
...
if (typeof ForceGraph3D === "undefined") {
  // Library never loaded. Don't proceed.
  document.getElementById("errbox").style.display = "block";
  document.getElementById("errbox").textContent = "ForceGraph3D not defined — CDN load failed. Check network, then reload.";
} else {
...
// Category buckets — group communities by topic so the legend reads as
// school / people / projects / ai / etc. instead of a flat hub list.
...
// HSL hash matching merge.py communityColor() for cross-viz consistency.
...
// Hide isolated nodes (deg=0) by default — at 11K-node scale they're orphans
// dragging on every animation frame. Settings toggle restores them.
let showIsolated = false;
const isIsolated = n => (n.deg || 0) === 0;

const Graph = ForceGraph3D()(document.getElementById("viz"))
  .graphData(DATA)
  ...
  .nodeLabel(n => `<div style="font-family:-apple-system,sans-serif;font-size:11.5px;background:rgba(11,13,18,.95);padding:6px 10px;border-radius:6px;border:1px solid rgba(255,255,255,.12);max-width:320px;backdrop-filter:blur(10px)"><strong style="color:#f3f5f9">${escapeHtml(n.label)}</strong><br><span style="color:#7aa6ff;font-size:10px">${escapeHtml(n.type)} · ${escapeHtml(communityName(n.community))} · deg ${n.deg}</span></div>`)
  .linkColor(() => "rgba(120,140,160,0.18)")
  .linkOpacity(0.18)
  .linkWidth(0.5)
  .linkDirectionalParticles(0)
  // Damping baked in — sim runs warmup ticks then cools to a stop. Once cool,
  // nothing moves. Re-heats only when a layout knob changes. Cooldown lowered
  // to 90 ticks (was 140) so the sim settles sooner at 11K-node scale.
  .warmupTicks(80)
  .cooldownTicks(90)
  .cooldownTime(6000)
  ...
  .onNodeClick(n => {
    ...
    Graph.cameraPosition(
      { x: (n.x || 1) * distRatio, y: (n.y || 1) * distRatio, z: (n.z || 1) * distRatio },
      n,
      900
    );
  })
  .onNodeHover(n => {
    ...
    // Trigger redraw — Obsidian-style fade-non-neighbors.
    Graph.nodeColor(Graph.nodeColor());
    Graph.linkColor(Graph.linkColor());
  })
  .onBackgroundClick(() => clearPin());

// Custom center-attract force — pulls every node toward origin proportional
// to its distance, matching Obsidian's "Center force" behavior. The built-in
// d3 forceCenter only translates the centroid (doesn't pull outliers in), so
// we add this as a separate force.
function makeCenterAttract(strength) {
  let nodes = [];
  let s = strength;
...
// Initial force tweaks — applied immediately, before settings panel binding.
// These run after .graphData() so the simulation is bound.
Graph.d3Force("charge").strength(-30);
Graph.d3Force("link").distance(28);
Graph.d3Force("centerAttract", makeCenterAttract(0.05));
// Built-in forceCenter still recenters the centroid — leave it but don't expose strength.
// Our custom centerAttract is what the user tunes.

// Track hovered node + adjacency for Obsidian-style highlight-on-hover.
let hoveredId = null;
function isHighlighted(nodeId) {
  if (!hoveredId) return true;
  if (nodeId === hoveredId) return true;
  return (adj.get(hoveredId) || new Set()).has(nodeId);
}
...
function focusCommunity(c) {
  // Average position of nodes in this community → fly camera to center.
  ...
  const dist = 220;
  Graph.cameraPosition({ x: cx + dist, y: cy + dist, z: cz + dist }, { x: cx, y: cy, z: cz }, 1200);
}
...
function pinNode(n) {
  ...
  info.innerHTML = `
    ...
    <div class="neighbors">
      <div class="title">neighbors (${neighbors.length}${(adj.get(n.id) || new Set()).size > 12 ? ` of ${adj.get(n.id).size}` : ""})</div>
  ...
  info.querySelectorAll("a[data-id]").forEach(a => {
    a.onclick = () => {
      ...
      if (target) { pinNode(target); Graph.cameraPosition(
        { x: (target.x || 1) * 1.5, y: (target.y || 1) * 1.5, z: (target.z || 1) * 1.5 },
        target, 800); }
    };
  });
}
...
document.getElementById("search-box").addEventListener("input", e => {
  ...
  if (hit) {
    pinNode(hit);
    if (hit.x != null) Graph.cameraPosition(
      { x: hit.x * 1.6, y: hit.y * 1.6, z: hit.z * 1.6 }, hit, 900);
  }
});
...
// Settings panel — live-tune visual params, persist to localStorage.
const DEFAULTS = {
  linkColor: "#7e8c9a", linkOpacity: 0.18, linkWidth: 0.5, linkParticles: false,
  nodeSize: 1.0, nodeMin: 0.5, nodeMax: 40, sizeCurve: "sqrt", nodeRes: 8, nodeOpacity: 0.92,
  charge: -30, linkDist: 28, center: 0.05,
  bg: "#0b0d12",
};
const SS_KEY = "brain-graph-3d-settings";
let settings = (() => {
...
function saveSettings() {
  try { localStorage.setItem(SS_KEY, JSON.stringify(settings)); } catch {}
}
...
function curveFn(d) {
  ...
  switch (settings.sizeCurve) {
    ...
    case "flat":   return 1;
    default:       return Math.sqrt(x);  // sqrt
  }
}
...
function applySettings(opts) {
  opts = opts || {};
  // Visual properties — safe to call any time. Each color/opacity callback
  // checks hoveredId to apply Obsidian-style highlight-on-hover fade.
  Graph
    .linkColor(link => {
    ...
    .nodeVal(nodeValFn)
    .nodeColor(n => {
      if (hoveredId && !isHighlighted(n.id)) return "#262b33";  // dim gray; node materials don't honor alpha in three.js
      return communityColor(n.community);
    })
    .nodeOpacity(settings.nodeOpacity)
    .nodeResolution(settings.nodeRes)
    .backgroundColor(settings.bg);
  // Force tweaks — only when triggered by user interaction (engine must be warm).
  if (opts.forces) {
    try {
      ...
      // Brief reheat so the new force takes effect, then natural cooldown stops the sim.
...
function bindSetting(id, key, parseVal, fmt) {
  ...
  el.value = (typeof settings[key] === "boolean") ? settings[key] : settings[key];
...
// Isolated-node toggle — same pattern as make-2d.py.
(function() {
...
bindSetting("s-node-res",       "nodeRes",       v => parseInt(v, 10), v => v.toString());
bindSetting("s-node-opacity",   "nodeOpacity",   parseFloat,        v => v.toFixed(2));
...
document.getElementById("s-reset").addEventListener("click", () => {
  settings = { ...DEFAULTS };
  saveSettings();
  // Re-bind all inputs to reflect defaults.
...
// Apply persisted visual settings after the engine has had a tick to warm up.
// Calling .nodeOpacity / .nodeVal / .linkColor immediately can blow up the
// renderObjs.tick() loop if the renderer hasn't finished mounting yet.
setTimeout(() => {
  try { applySettings({ forces: false }); }
  catch (e) { console.warn("initial applySettings failed:", e); }
  // Force tweaks deferred slightly more so the simulation is fully bound.
  setTimeout(() => {
    try { applySettings({ forces: true }); }
    catch (e) { console.warn("force apply failed:", e); }
  }, 400);
}, 100);
}
</script>
